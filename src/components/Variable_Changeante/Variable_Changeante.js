...
import { useFees, useFeesById, useDeleteFeesById, useCreateFees, useModifyFees } from "../../Hooks/api/fees.api";
import { useAuthDispatch } from "../../stores/auth.store.js";
export const Variable_Changeante = () => {
    // work
    const FeesQuery = useFees();
    const FeeByIdQuery = useFeesById();
    const FeeDeleteByIdQuery = useDeleteFeesById();
    const FeeCreateQuery = useCreateFees();
    const FeeModifyQuery = useModifyFees();
    ...
    const saveProduct = async (e) => {
        ...
        if (product.id == null) {
            try {
                await FeeCreateQuery.mutateAsync(formData);
                history.push({
        ...
        <div className="grid crud-demo">
            <div className="col-12">
                <div className="card">
                    ...
                    {FeesQuery.isIdle || FeesQuery.isLoading ? (
                        <>
                            {[1, 2, 3, 4].map((n) => (
                                <DataTable value={products} className="p-datatable-striped">
                                    <Column selectionMode="multiple" headerStyle={{ width: "3rem" }}></Column>
                                    <Column field="code" header="TVA" sortable body={skeletonTemplate} headerStyle={{ width: "50%", minWidth: "10rem" }} className="bg-cyan-400 border-round-top"></Column>
                                    <Column field="name" header="Timbre" sortable body={skeletonTemplate} headerStyle={{ width: "50%", minWidth: "10rem" }} className="bg-indigo-300 border-round-top"></Column>
                    ...
                    <Dialog visible={productDialog} style={{ width: "450px" }} header="Product Details" modal className="p-fluid" footer={productDialogFooter} onHide={hideDialog}>
                        ...
                        <div className="field">
                            ...
                            <InputText id="TVA" value={product.tva} onChange={(e) => onInputChange(e, "tva")} required autoFocus className={classNames({ "p-invalid": submitted && !product.tva })} />
                    ...
                    <Dialog visible={factureDialog} style={{ width: "450px" }} header="Product Details" modal className="p-fluid" footer={productDialogFooter} onHide={hideDialog}>
                        {FeeByIdQuery.isIdle || FeeByIdQuery.isLoading ? (
                            <>
                                <skeletonTemplate />
                            </>
                        ) : FeeByIdQuery.isSuccess ? (
                            ""
                        ) : (
                            ""
                        )}
