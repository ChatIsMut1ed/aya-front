...
import { useMat, useMatById, useDeleteMatById, useCreateMat, useModifyMat } from "../../Hooks/api/matiere.api";
import { useAuthDispatch } from "../../stores/auth.store.js";
export const Mati = () => {
    // work
    const MatsQuery = useMat();
    const MatByIdQuery = useMatById();
    const MatDeleteByIdQuery = useDeleteMatById();
    const MatCreateQuery = useCreateMat();
    const MatModifyQuery = useModifyMat();
    ...
    const saveProduct = async (e) => {
        ...
        if (product.id == null) {
            try {
                await MatCreateQuery.mutateAsync(formData);
                history.push({
                    pathname: "/Mati",
                });
    ...
    const deleteProduct = async (e) => {
        ...
        try {
            await MatDeleteByIdQuery.mutateAsync(product.id);
            history.push({
                pathname: "/DevisMat",
            });
            setProducts(products);
        ...
        <div className="grid crud-demo">
            <div className="col-12">
                <div className="card">
                    ...
                    {MatsQuery.isIdle || MatsQuery.isLoading ? (
                        <>
                            {[1, 2, 3, 4].map((n) => (
                                <DataTable value={products} className="p-datatable-striped">
                                    <Column selectionMode="multiple" headerStyle={{ width: "3rem" }}></Column>
                                    <Column field="code" header="Designation" sortable body={skeletonTemplate} headerStyle={{ width: "30%", minWidth: "10rem" }} className="bg-cyan-400 border-round-top"></Column>
                                    <Column field="name" header="Unité" sortable body={skeletonTemplate} headerStyle={{ width: "30%", minWidth: "10rem" }} className="bg-indigo-300 border-round-top"></Column>
                                    <Column field="price" header="Prix Unitaire" body={skeletonTemplate} sortable headerStyle={{ width: "30%", minWidth: "8rem" }} className="bg-pink-200 border-round-top"></Column>
                        ...
                        >
                            <Column selectionMode="multiple" headerStyle={{ width: "3rem" }}></Column>
                            <Column field="code" header="Designation" sortable body={codeBodyTemplate} headerStyle={{ width: "30%", minWidth: "10rem" }} className="bg-cyan-400 border-round-top"></Column>
                            <Column field="name" header="Unité" sortable body={nameBodyTemplate} headerStyle={{ width: "30%", minWidth: "10rem" }} className="bg-indigo-300 border-round-top"></Column>
                            <Column field="price" header="Prix Unitaire" body={priceBodyTemplate} sortable headerStyle={{ width: "30%", minWidth: "8rem" }} className="bg-pink-200 border-round-top"></Column>
                    ...
                    <Dialog visible={productDialog} style={{ width: "450px" }} header="Product Details" modal className="p-fluid" footer={productDialogFooter} onHide={hideDialog}>
                        ...
                        <div className="field">
                            ...
                            <InputText id="Designation" value={product.designation} onChange={(e) => onInputChange(e, "designation")} required autoFocus className={classNames({ "p-invalid": submitted && !product.designation })} />
                            {/* {submitted && !product.name && <small className="p-invalid">Designation is required.</small>} */}
                        </div>
                        <div className="field">
                    ...
                    <Dialog visible={factureDialog} style={{ width: "450px" }} header="Product Details" modal className="p-fluid" footer={productDialogFooter} onHide={hideDialog}>
                        {MatByIdQuery.isIdle || MatByIdQuery.isLoading ? (
                            <>
                                <skeletonTemplate />
                            </>
                        ) : MatByIdQuery.isSuccess ? (
                            ""
                        ) : (
                            ""
                        )}
