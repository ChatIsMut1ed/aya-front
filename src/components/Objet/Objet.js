...
import { useObject, useObjectById, useDeleteObjectById, useCreateObject, useModifyObj } from "../../Hooks/api/object.api";
import { useAuthDispatch } from "../../stores/auth.store.js";
export const Objet = () => {
    // work
    const ObjsQuery = useObject();
    const ObjByIdQuery = useObjectById();
    const ObjDeleteByIdQuery = useDeleteObjectById();
    const ObjCreateQuery = useCreateObject();
    const ObjModifyQuery = useModifyObj();
    ...
    const saveProduct = async (e) => {
        ...
        if (product.id == null) {
            try {
                await ObjCreateQuery.mutateAsync(formData);
                history.push({
                    pathname: "/Objet",
                });
    ...
    const deleteProduct = async (e) => {
        ...
        try {
            await ObjDeleteByIdQuery.mutateAsync(product.id);
            history.push({
                pathname: "/Objet",
            });
            setProducts(products);
        ...
        <div className="grid crud-demo">
            <div className="col-12">
                <div className="card">
                    ...
                    {ObjsQuery.isIdle || ObjsQuery.isLoading ? (
                        <>
                            {[1, 2, 3, 4].map((n) => (
                                <DataTable value={products} className="p-datatable-striped">
                                    <Column selectionMode="multiple" headerStyle={{ width: "3rem" }}></Column>
                                    <Column field="code" header="Designation" sortable body={skeletonTemplate} headerStyle={{ width: "20%", minWidth: "10rem" }} className="bg-cyan-400 border-round-top"></Column>
                                    <Column field="name" header="Unité" sortable body={skeletonTemplate} headerStyle={{ width: "20%", minWidth: "10rem" }} className="bg-indigo-300 border-round-top"></Column>
                                    <Column field="price" header="Prix_Unitaire" body={skeletonTemplate} sortable headerStyle={{ width: "20%", minWidth: "8rem" }} className="bg-pink-200 border-round-top"></Column>
                                    <Column header="Superficie" body={skeletonTemplate} headerStyle={{ width: "20%", minWidth: "10rem" }} className="bg-green-300 border-round-top"></Column>
                                    <Column field="QTS/H" header="QTS/H" body={skeletonTemplate} sortable headerStyle={{ width: "20%", minWidth: "8rem" }} className="bg-pink-200 border-round-top"></Column>
                        ...
                        >
                            <Column selectionMode="multiple" headerStyle={{ width: "3rem" }}></Column>
                            <Column field="code" header="Designation" sortable body={codeBodyTemplate} headerStyle={{ width: "20%", minWidth: "10rem" }} className="bg-cyan-400 border-round-top"></Column>
                    ...
                    <Dialog visible={factureDialog} style={{ width: "450px" }} header="Product Details" modal className="p-fluid" footer={productDialogFooter} onHide={hideDialog}>
                        {ObjByIdQuery.isIdle || ObjByIdQuery.isLoading ? (
                            <>
                                <skeletonTemplate />
                            </>
                        ) : ObjByIdQuery.isSuccess ? (
                            ""
                        ) : (
                            ""
                        )}
